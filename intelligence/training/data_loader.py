...
def fetch_single_dhan_request(client, security_id, start_date, end_date, timeframe):
    """
    Make a single Dhan API request (existing logic moved here)
    """
    try:
        ...
        # For intraday requests, add time component as required by Dhan API v2
        if timeframe != "1day":
            from_date_with_time = f"{from_date} 09:30:00"
            to_date_with_time = f"{to_date} 15:30:00"
        else:
            from_date_with_time = from_date
            to_date_with_time = to_date
        ...
        if timeframe == "1day":
            ...
            try:
                historical_data = client.historical_daily_data(
                    ...
                    from_date=from_date_with_time,
                    to_date=to_date_with_time
                )
            except Exception as e:
                error_str = str(e).lower()
                if "rate limit" in error_str or "429" in error_str or "too many requests" in error_str:
                    print(f"[WARNING] Rate limit hit during daily data request, waiting and retrying once...")
                    time.sleep(2.0)  # Brief wait
                    try:
                        historical_data = client.historical_daily_data(
                            security_id=security_id,
                            exchange_segment="NSE_EQ",
                        ...
                        print(f"[WARNING] Retry failed for daily data: {retry_e}")
                        return None
                else:
                    print(f"[WARNING] Daily data request failed: {e}")
                    return None
        else:
            ...
            try:
                try:
                    ...
                    error_str = str(e).lower()
                    if "rate limit" in error_str or "429" in error_str or "too many requests" in error_str:
                        print(f"[WARNING] Rate limit hit during intraday data request, waiting and retrying once...")
                        time.sleep(2.0)  # Brief wait
                        try:
                            historical_data = client.intraday_minute_data(
                                security_id=security_id,
                            ...
                            print(f"[WARNING] Retry failed for intraday data: {retry_e}")
                            return None
                    else:
                ...
                print(f"[WARNING] Intraday data not available, using daily data")
                try:
                    historical_data = client.historical_daily_data(
                        security_id=security_id,
                        exchange_segment="NSE_EQ",
                    ...
                    error_str = str(e).lower()
                    if "rate limit" in error_str or "429" in error_str or "too many requests" in error_str:
                        print(f"[WARNING] Rate limit hit during fallback daily data request, waiting and retrying once...")
                        time.sleep(2.0)  # Brief wait
                        try:
                            historical_data = client.historical_daily_data(
                                security_id=security_id,
                            ...
                            print(f"[WARNING] Retry failed for fallback daily data: {retry_e}")
                            return None
                    else:
                        print(f"[WARNING] Fallback daily data request failed: {e}")
                        return None
            except Exception as e:
                error_str = str(e).lower()
                if "rate limit" in error_str or "429" in error_str or "too many requests" in error_str:
                    print(f"[WARNING] Rate limit hit, but retry already attempted")
                    return None
                else:
                    print(f"[WARNING] Intraday data request failed: {e}")
                    return None
        
        if not historical_data:
            print(f"[WARNING] No data returned from Dhan")
            return None
        ...
        if isinstance(historical_data, dict) and 'data' in historical_data:
            ...
            # Always print error details if present
            if isinstance(historical_data['data'], dict):
                err = historical_data['data']
                if any(k in err for k in ['errorType', 'errorCode', 'errorMessage']):
                    print(f"[DHAN ERROR] errorType: {err.get('errorType')}, errorCode: {err.get('errorCode')}, errorMessage: {err.get('errorMessage')}")
            if not actual_data:
                print(f"[WARNING] Empty data section in Dhan response")
                return None
        ...
        if all(key in actual_data for key in ['open', 'high', 'low', 'close', 'timestamp']):
            ...
            print(f"[WARNING] Unexpected data format from Dhan")
        ...
        print(f"[WARNING] Single Dhan request failed: {e}")
        return None

def fetch_dhan_with_chunking(client, security_id, start_date, end_date, timeframe):
    """
    Fetch Dhan data with automatic chunking for intraday timeframes
    Returns combined DataFrame from all chunks
    """
    if timeframe == "1day":
        # No chunking needed for daily data
        return fetch_single_dhan_request(client, security_id, start_date, end_date, timeframe)
    ...
    if total_days <= TradingConfig.DHAN_MAX_DAYS_INTRADAY:
        # Single request sufficient
        return fetch_single_dhan_request(client, security_id, start_date, end_date, timeframe)
    
    # Multiple chunks needed
    chunks_needed = (total_days // TradingConfig.DHAN_MAX_DAYS_INTRADAY) + 1
    print(f"[INFO] Large date range ({total_days} days) requires {chunks_needed} chunks for intraday data")
    ...
    for chunk_num in range(chunks_needed):
        ...
        chunk_end = min(current_start + timedelta(days=TradingConfig.DHAN_MAX_DAYS_INTRADAY), end_dt)
        
        chunk_start_str = current_start.strftime('%Y-%m-%d')
        chunk_end_str = chunk_end.strftime('%Y-%m-%d')
        
        print(f"[INFO] Fetching chunk {chunk_num + 1}/{chunks_needed}: {chunk_start_str} to {chunk_end_str}")
        
        # Fetch chunk data
        chunk_df = fetch_single_dhan_request(client, security_id, chunk_start_str, chunk_end_str, timeframe)
        
        if chunk_df is not None and not chunk_df.empty:
            all_chunks.append(chunk_df)
        
        # Rate limiting between chunks
        if chunk_num < chunks_needed - 1:  # Don't sleep after last chunk
            time.sleep(TradingConfig.DHAN_REQUEST_DELAY)
    ...
    if all_chunks:
        ...
        print(f"[✓] Combined {len(all_chunks)} chunks into {len(combined_df)} total rows")
        return combined_df
    else:
        print(f"[WARNING] No data retrieved from any chunks")
        return None

def fetch_from_dhan(ticker, start_date, end_date, timeframe="1day"):
    """
    Fetch data from Dhan with configurable timeframe and automatic chunking
    """
    ...
    try:
        ...
        print(f"[INFO] Fetching Dhan data for {ticker} (ID: {security_id}) from {start_date} to {end_date}, timeframe: {timeframe}")
        start_time = time.time()
        
        # Use chunking logic
        df = fetch_dhan_with_chunking(client, security_id, start_date, end_date, timeframe)
        
        elapsed_time = time.time() - start_time
        if df is not None and not df.empty:
            print(f"[✓] Fetched {len(df)} rows from Dhan for {ticker} in {elapsed_time:.2f}s")
        else:
            print(f"[WARNING] No data retrieved for {ticker} after {elapsed_time:.2f}s")
        
        # Final rate limiting (applied once per ticker, regardless of chunks)
        time.sleep(TradingConfig.DHAN_REQUEST_DELAY)
        ...
        # Rate limiting for Dhan API (even for failed requests)
        time.sleep(TradingConfig.DHAN_REQUEST_DELAY)
        return None
...
def download_nse_data(tickers: list[str], start="2020-01-01", end="2024-01-01", timeframe: str = "1day") -> dict[str, pd.DataFrame]:
    ...
    # Load the NSE mapping at the start (for info/logging only)
    mapping = load_nse_equity_mapping()
    if mapping:
        print(f"[INFO] NSE equity mapping loaded with {len(mapping)} symbols")
    else:
        print(f"[WARNING] NSE equity mapping not available, will still try Dhan for all tickers")
    
    for i, ticker in enumerate(tickers, 1):
        print(f"[INFO] Processing {ticker} ({i}/{len(tickers)})...")
        ticker_start_time = time.time()
        
        # Always try Dhan first
        df = fetch_from_dhan(ticker, start, end, timeframe)
        
        if df is not None and not df.empty and "Close" in df.columns:
            ticker_elapsed = time.time() - ticker_start_time
            print(f"[✓] {ticker} completed via Dhan in {ticker_elapsed:.2f}s")
            result[ticker] = df
            dhan_success += 1
            continue
        ...
        if df is not None and not df.empty:
            if "Close" not in df.columns:
                ...
                continue
            ticker_elapsed = time.time() - ticker_start_time
            print(f"[✓] {ticker} completed via yFinance in {ticker_elapsed:.2f}s")
            result[ticker] = df
            yfinance_fallback += 1
        else:
            ticker_elapsed = time.time() - ticker_start_time
            print(f"[⚠️] Failed to fetch data for {ticker} from both sources after {ticker_elapsed:.2f}s")
    
    batch_elapsed = time.time() - batch_start_time
    print(f"[INFO] Data fetch complete in {batch_elapsed:.2f}s:")
    print(f"  - Dhan: {dhan_success} tickers")  
    print(f"  - yFinance: {yfinance_fallback} tickers")
    print(f"  - Total: {len(result)} tickers")
    print(f"  - Average time per ticker: {batch_elapsed/len(tickers):.2f}s")
